...
def createTeam(firstIndex, secondIndex, isRed,
               first = 'AttackerPDDL', second = 'DefenderPDDL'):
  """
...
class AttackerPDDL(CaptureAgent):
  """
  ...
  def createPDDLobjects(self, gameState):
    ...
    # First build the PDDL objects; the positions
  ...
  def solvePddl(self):
    ...
    data = {'domain': open('pacman-domain.pddl', 'r').read(),'problem': open("problem"+ str(self.index + 1) + ".pddl", 'r').read()}
...
class DefenderPDDL(CaptureAgent):
  """
  A Dummy agent to serve as an example of the necessary agent structure.
  You should look at baselineTeam.py for more details about how to
  create an agent as this is the bare minimum.
  ...
  def solvePddl(self):
    ...
    action = 'Stop'
    data = {'domain': open('ghost-domain.pddl', 'r').read(), 'problem': open("problem"+ str(self.index + 1) + ".pddl", 'r').read()}
    req = urllib2.Request('http://solver.planning.domains/solve')
    req.add_header('Content-Type', 'application/json')
    resp = json.loads(urllib2.urlopen(req, json.dumps(data)).read())
    print '\n496\n', resp, '\n\n'
